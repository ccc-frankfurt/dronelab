            ...
            try:
                ...
                if self.response_state != 'ok':
                    ...
                    with open ("/home/pi/dronelab/djitellopy/string.txt", "w") as file:
                        file.write(str(list))
                    self.pitch = int(list[5].split(":")[1])
                    self.roll = int(list[6].split(":")[1])
                    self.yaw = int(list[7].split(":")[1])
                    self.speed_x = int(list[8].split(":")[1])
                    self.speed_y = int(list[9].split(":")[1])
                    self.speed_z = int(list[10].split(":")[1])
                    self.temperature_lowest = int(list[11].split(":")[1])
                    self.temperature_highest = int(list[12].split(":")[1])
                    self.distance_tof = int(list[13].split(":")[1])
                    self.height = int(list[14].split(":")[1])
                    self.battery = int(list[15].split(":")[1])
                    self.barometer = float(list[16].split(":")[1])
                    self.flight_time = float(list[17].split(":")[1])
